  ...
  const onSubmitHander = async (e) => {
    ...
    try {
      ...
      if (object.split(' ').length > 1) {
        return toast('Please return only one object name')
      }
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHander} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <textarea rows={4} onChange={(e) => setInput(e.target.value)} value={input} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='e.g., watch or spoon, Only single object name' required />
        ...
        </div>
        {
          !content ? (
            <div className='flex-1 flex justify-center items-center'>
              <div className='text-sm flex flex-col items-center gap5 text-gray-400'>
